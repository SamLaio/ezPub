...
4. 產生一份 release 說明草稿，檔名以版本號分界，例如 `release-notes/0.1.2.md`。
...
1. 確認版本號已更新。
2. 執行 `go test ./...`。
3. 檢查是否已有該版本 release 說明檔，例如 `release-notes/0.1.2.md`。
4. 如果沒有 release 說明檔，先產生一份。
5. 確認 `release/` 資料夾下已有該版本的 exe，例如 `release/ezpub-0.1.2-windows-amd64.exe`。
6. exe 一律輸出到 `release/`。
7. exe 檔名一律包含版本號與平台，例如 `ezpub-0.1.2-windows-amd64.exe`。
...
10. 發布前，將該版本 exe 與整個 `release/tools/` 資料夾壓縮成 zip，例如 `release/ezpub-0.1.2-windows-amd64.zip`：

```powershell
Compress-Archive -Force -Path .\release\ezpub-0.1.2-windows-amd64.exe, .\release\tools -DestinationPath .\release\ezpub-0.1.2-windows-amd64.zip
```
