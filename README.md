...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe version
```

開啟 GUI：

```powershell
.\release\ezpub-0.3.0-windows-amd64.exe
```

也可以直接帶入 TXT：

```powershell
.\release\ezpub-0.3.0-windows-amd64.exe gui ".\test_file\巴黎茶花女遺事 - 小仲馬.txt"
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build .\examples\sample.txt -o .\examples\sample.epub
```

使用舊 EasyPub 設定檔：

```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build novel.txt -o novel.epub -config C:\PortableApps\easypub\config.xml
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe chapters novel.txt -o novel.chapters.tsv
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build novel.txt -o novel.epub -chapters novel.chapters.tsv
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build novel.txt -o novel.epub -split-length 12000
```

相容 EasyPub 的「按長度均分 x 章」：

```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build novel.txt -o novel.epub -split-count 10
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe images .\illustrations -o images.tsv
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe images .\illustrations -copy-ref 001.jpg
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe images .\illustrations -open 001.jpg
```

建書時套用插圖清單：

```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build novel.txt -o novel.epub -image-list images.tsv
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build novel.txt -o novel.epub -scan-images .\illustrations
```

release 版會在 `tools/pyftsubset.exe` 隨附 fontTools 的 `pyftsubset`。找到這個工具時，可在輸出前子集化內嵌字型：

```powershell
.\release\ezpub-0.3.0-windows-amd64.exe build novel.txt -o novel.epub -font .\font.ttf -subset-fonts
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe txt novel.epub -o novel.txt
```
...
```powershell
.\release\ezpub-0.3.0-windows-amd64.exe inspect-config C:\PortableApps\easypub\config.xml
```
